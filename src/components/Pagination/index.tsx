...
const Pagination = (props: Props) => {
  ...
  const handleRowOnPage = (type: TypeCount) => {
    ...
    crrDataPagination.rowOnPage =
      type === TypeCount.INCR
        ? crrDataPagination.rowOnPage + 1
        : crrDataPagination.rowOnPage > 1
        ? crrDataPagination.rowOnPage - 1
        : crrDataPagination.rowOnPage;
    if (prevValue !== crrDataPagination.rowOnPage) {
      ...
      localStorage.setItem(
        "currentTotalRowOnPage",
        String(crrDataPagination.rowOnPage)
      );
    }
  };
  ...
  return (
    <div className={`${styles.paginationAjax} ${props.className || ""}`}>
      <div className={styles.rowOnPage}>
        <div className={`${styles.chevronRoot} bgWhite`}>
          <div className={styles.chevron}>
            <div
              className={`${styles.chevronUp} ${styles.btnHanldePage} `}
              onClick={() => {
            ...
            <div
              className={`${styles.chevronDown} ${styles.btnHanldePage} `}
              onClick={() => {
          ...
          <span>
            {crrDataPagination.rowOnPage < 10
              ? `0${crrDataPagination.rowOnPage}`
              : crrDataPagination.rowOnPage}
          </span>
        </div>
      ...
      <div className={styles.pagination}>
        <div
          className={`${styles.prevPage} ${styles.btnHanldePage} ${
            crrDataPagination.crrPage < 2 ? styles.disabled : ""
          }`}
          onClick={() => {
        ...
        <span className={styles.currentPage}>
          {crrDataPagination.crrPage < 10
            ? `0${crrDataPagination.crrPage}`
            : crrDataPagination.crrPage}
        </span>
        <div
          className={`${styles.nextPage} ${styles.btnHanldePage} ${
            crrDataPagination.crrPage === props.maxPage ? styles.disabled : ""
          }`}
          onClick={() => {
        ...
        <div className={styles.page}>
          <span>
            {crrDataPagination.crrPage < 10
              ? `0${crrDataPagination.crrPage}`
              : crrDataPagination.crrPage}
            {props.showSizePage ? `/${props.maxPage}` : ""}
          </span>
          <div className={styles.chevron}>
            <div
              className={`${styles.chevronUp} ${styles.btnHanldePage} `}
