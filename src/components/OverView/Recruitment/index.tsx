...
const OverViewRecruitment = (props: Props) => {
    ...
    const getPrevMonth = new Date(currentDate.getFullYear(), currentDate.getMonth() - 1, 1);
    ...
    const handleChangeDate = (value: Obj | null, type: 'START_DATE' | 'END_DATE') => {
        if (value !== null) {
    ...
    return (
        <div className={`${props.isOnExpand ? styles.recruitmentOnExpand : styles.overViewRecruitment}`}>
            ...
            <div className={`${styles.pickDate}`}>
                Lọc: <DatePicker value={dayjs(conditionDate.startDate)} size="small" placeholder="Từ ngày" format="DD/MM/YYYY" onChange={(value) => {
                    handleChangeDate(value, 'START_DATE')
                }} />
                <span className={styles.arrow}>{MapIconKey[KEY_ICON.ARROWL]}</span>
                <DatePicker value={dayjs(conditionDate.endDate)} size="small" placeholder="Đến ngày" format="DD/MM/YYYY" onChange={(value) => {
