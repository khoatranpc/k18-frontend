...
const TableIndex = () => {
  ...
  const getDataListCandidate =
    ((listCandidate.data.response?.data as Obj)?.listData as Obj[]) || [];
  ...
  const calcAge = getDataListCandidate.reduce(
    (value, item) => {
      ...
      if (item.roundProcess === RoundProcess.CV && StatusProcessing.DONE) {
        if (item.result === ResultInterview.NOTPASS) {
          averageTimeProcess["TIME"] += countTime(
            createdAt,
            item.failCVDate || new Date()
          );
          averageTimeProcess["COUNTTIME"]++;
      ...
      if (
        item.roundProcess !== RoundProcess.CV &&
        item.roundProcess !== RoundProcess.INTERVIEW
      ) {
      ...
      info: [
        {
          ...
          rate: `${Number(
            (mapDataTableIndex.PASSPV / getDataListCandidate.length) * 100 || 0
          ).toFixed(2)}%`,
        },
        {
          tt: "ĐÁNH GIÁ",
          ...
          rate: `${Number(
            (mapDataTableIndex.PENDINGPV / getDataListCandidate.length) * 100 ||
              0
          ).toFixed(2)}%`,
        },
      ],
      ...
      info: [
        {
          ...
          rate: `${Number(
            (mapDataTableIndex.NOPROCESS / getDataListCandidate.length) * 100 ||
              0
          ).toFixed(2)}%`,
        },
      ],
      ...
      info: [
        {
          ...
          rate: `${Number(
            (mapDataTableIndex.FAILCV / getDataListCandidate.length) * 100 || 0
          ).toFixed(2)}%`,
        },
        {
          content: "Fail PV",
          ...
          rate: `${Number(
            (mapDataTableIndex.FAILPV / getDataListCandidate.length) * 100 || 0
          ).toFixed(2)}%`,
        },
      ],
      ...
      <table className={styles.tableTotal}>
        <thead>
          <tr>
            ...
            <th rowSpan={2} className={`${styles.bdLeft} ${styles.bdBottom}`}>
              Nam: {getGender.MALE}
            </th>
            <th
              rowSpan={4}
              className={`${styles.bdLeft} text-center ${styles.avgProcess}`}
            >
              {Number(
                averageTimeProcess["TIME"] / averageTimeProcess["COUNTTIME"] ||
                  0
              ).toFixed(2)}{" "}
              Ngày
            </th>
          ...
          <tr></tr>
          <tr>
            <th rowSpan={2} className={styles.ageAvg}>
              Độ tuổi TB:{" "}
              {Number(calcAge.dob / getDataListCandidate.length || 0).toFixed(
                2
              )}
            </th>
        ...
        <tbody>
          <tr className={styles.title}>
            <td colSpan={2}>Tổng dữ liệu ứng viên</td>
            <td>Thời gian xử lý TB</td>
          </tr>
        </tbody>
      </table>
      <Table
