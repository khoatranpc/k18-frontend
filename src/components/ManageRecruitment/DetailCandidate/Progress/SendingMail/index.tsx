...
const SendingMail = (props: Props) => {
  const mapRoundToMailTemplate: Record<string, TemplateMail> = {
    ...
    [RoundProcess.INTERVIEW]: !props.pass
      ? TemplateMail.FAILINTERVIEW
      : TemplateMail.PASSINTERVIEW,
    ...(props.noConnect && {
  ...
  const getTemplate =
    mapRoundToMailTemplate[
      props.round ? props.round : props.noConnect ? "NOCONNECT" : ""
    ];
  ...
  return (
    <div className={styles.sendingMail}>
      <Button
        size="small"
        className={styles.btnSentMailCv}
        onClick={() => {
          setModal(true);
        }}
      >
        <Send className={styles.iconSent} />
        Gửi mail
