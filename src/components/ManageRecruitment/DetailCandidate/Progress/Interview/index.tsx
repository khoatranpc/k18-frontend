    ...
    return (
        <div className={styles.roundInterview}>
            <div className={`${styles.handleInterview} ${styles.infoRound}`}>
                <h2>Vòng phỏng vấn {(<sup style={{ color: !getDataRoundProcess?.result ? 'var(--light-red)' : 'var(--success)' }}>{!getDataRoundProcess?.processed ? 'Đang xử lý' : (getDataRoundProcess?.result ? 'Pass' : 'Failed')}</sup>)}</h2>
                <div className={styles.infoInterview}>
                    ...
                    <p>TE: {getDataRoundProcess?.te ? (`${getDataRoundProcess?.te.teName}-${getDataRoundProcess?.te.positionTe}${getDataRoundProcess?.te.courseId ? ` ${(getDataRoundProcess?.te?.courseId as Obj[])?.map((item) => item.courseName)?.toString()}` : ''}`) : (<span className="error">Chưa có thông tin TE!</span>)}</p>
                </div>
                <div className={styles.function}>
                    <div className={styles.actions}>
                        ...
                        <span className={styles.sentMail}>
                            <SendingMail pass={getDataRoundProcess?.result} statusSendMail={getDataRoundProcess?.mailResultSent} round={RoundProcess.INTERVIEW} roundId={getDataRoundProcess?._id} />
                        </span>
                    </div>
