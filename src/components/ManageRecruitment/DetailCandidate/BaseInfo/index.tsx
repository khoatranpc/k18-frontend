...
const BaseInfo = () => {
  ...
  const handleSendmail = (template: TemplateMail, payload: Obj) => {
    const mailInfo = {
  ...
  return (
    <div className={styles.containerBaseInfo}>
      ...
      <div>
        <h2>Hành động gửi Mail</h2>

        <SendingMailV2
          round={TemplateMail.NOCONNECT}
          text={"Liên hệ nhiều lần"}
          handleSend={handleSendmail}
          isLoading={sendMailCandidate.data.isLoading}
          statusSendMail={detailCandidate?.sendMailNoConnect}
        />

        <SendingMailV2
          round={TemplateMail.PENDING}
          text={"Ngưng Tuyển"}
          handleSend={handleSendmail}
          isLoading={sendMailCandidate.data.isLoading}
          statusSendMail={detailCandidate?.sendMailPending}
        />
      </div>
    </div>
  );
