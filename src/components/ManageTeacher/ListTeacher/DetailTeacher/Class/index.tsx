...
import FormCreateEvaluate from "@/components/TeacherEvaluate/FormCreateEvaluate";
...
const Class = () => {
  ...
  const getListFeedback =
    (listFeedback.data.response?.data?.list as Obj[]) ?? [];
  ...
  const colums: Columns = [
    {
      ...
      render(value) {
        return (
          <span className="flex align-center" style={{ gap: "0.8rem" }}>
            {value?.codeClass || ""} <EyeFilled />
          </span>
        );
      },
      onCell(record) {
        return {
          onClick() {
            router.push(
              `/te/manager/class/detail/${
                (record.classId as Obj)?._id as string
              }`
            );
            const headerComponent = (
              <TitleHeader
                ...
                editTitle
                title={(record.classId as Obj)?.codeClass as string}
                dateStart={formatDatetoString(
                  new Date((record.classId as Obj)?.dayRange?.start as Date),
                  "dd/MM/yyyy"
                )}
                statusClass={(record.classId as Obj)?.status as STATUS_CLASS}
              />
            );
            dispatchRouter(
              CombineRoute["TE"]["MANAGER"]["DETAILCLASS"],
              (record.classId as Obj)?.codeClass as string,
              headerComponent,
              ComponentPage.DETAILCLASS,
              true
            );
          },
        };
      ...
      render(value) {
        return (
          <div
            ...
            style={{
              backgroundColor:
                getColorFromStatusClass[value?.status as STATUS_CLASS],
            }}
          >
    ...
    },
    {
      ...
      render(value, record) {
        return `Nhóm ${value} - ${
          (record.locationId as Obj)?.locationCode as string
        }`;
      },
    },
    {
      ...
      dataIndex: "classId",
      render(value) {
        const getTimeScheduleData = getMatchingTimeSchedule(
          listTimeSchedule.data.response?.data as Array<Obj>,
          value?.timeSchedule as Array<Obj>
        );
        return getTimeScheduleData.map((item) => {
          return (
            <p key={item.order as number} style={{ margin: 0 }}>
    ...
    },
    {
      ...
      render(value, record: Obj) {
        ...
        const fbClasses = getListFeedback.filter((item) => {
          return (
            item.codeClass?._id === classId &&
            item.groupNumber?.groupNumber === record.groupNumber
          );
        });
        let tc = 0;
        if (fbClasses.length) {
        ...
        return (
          <Badge
            count={
              fbClasses.length
                ? Number(tc / fbClasses.length).toLocaleString(undefined, {
            ...
            color={
              fbClasses.length
                ? getColorTeacherPoint(tc / fbClasses.length)
                : "var(--base)"
            }
          >
            <span className={styles.positionTeacher}>
              {(getTeacher?.roleRegister as ROLE_TEACHER) || ""}
            </span>
          </Badge>
        );
      },
    },
    {
      ...
      render(value) {
        ...
        return (
          <span className={styles.iconEnroll}>
            {(crrTeacher?.enroll
              ? MapIconKey[KEY_ICON.TICK]
              : MapIconKey[KEY_ICON.CL]) || ""}
          </span>
        );
      },
    },
  ...
  useEffect(() => {
    listClass.query(router.query.teacherId as string, [
      "_id",
      "classId",
      "dayRange",
      "start",
