...
const ContainerPage = (props: Props) => {
  ...
  const items: MenuItem[] = mappingTab?.map((item) => {
    return !item.hide &&
      item.positionAccept?.includes(
        crrRole === ROLE_USER.TE ? (getPosition as PositionTe) : crrRole
      )
      ? getItem(
          item.title,
          item.route,
  ...
  useEffect(() => {
    if (
      (crrUser.response?.data as Obj) &&
      Object.keys(crrUser.response?.data as Obj).length === 0
    ) {
    ...
    if (crrRole) {
      if (
        !getRolePage.includes(ROLE_USER.COMMON) &&
        (!getRolePage.includes(crrRole) ||
          (crrRole === ROLE_USER.TE &&
            !getRolePage.includes(getPosition) &&
            !getRolePage.includes(ROLE_USER.COMMON)))
      ) {
        router.push("/404");
      } else {
        ...
        >
          <div className={styles.badge}>
            <Avatar
              size="large"
              src={
                ((crrUser.response as Obj)?.data?.img as string) ??
                ((crrUser.response as Obj)?.data?.image as string) ??
                "/static/logo.png"
              }
            />
            {!collapsed && (
              <div className={styles.user}>
                <p>
                  {((crrUser.response as Obj)?.data?.teName as string) ??
                    ...
                    ((crrUser.response as Obj)?.data?.name as string)}
                </p>
                <span className={styles.role}>
                  {((crrUser.response as Obj)?.data
                    ?.roleAccount as ROLE_USER) === ROLE_USER.TE
                    ? `${
                        getLabelPositionTe[
                          (crrUser.response as Obj)?.data
