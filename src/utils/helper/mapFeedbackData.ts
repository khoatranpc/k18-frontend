...
function mapFeedbackData(classData: ClassData[], feedbackData: FeedbackData[]): Output[] {
    return classData.map(classItem => {
        ...
        const totalPoints = feedbacks.reduce((sum, feedback) => {
            return sum + ((feedback.pointST || 0) + (feedback.pointMT || 0)) / 2;
        }, 0);
        ...
        const listFb: OutputFeedback[] = feedbacks.map(feedback => ({
            ...
            pointCxo: feedback.pointCxo || 0,
            pointMT: feedback.pointMT || 0,
            pointOb: feedback.pointOb || 0,
            pointST: feedback.pointST || 0,
            pointSyl: feedback.pointSyl || 0,
            timeCollect: feedback.timeCollect || 0,
        ...
        const teacherMTTotal = teacherMTFeedbacks.reduce((sum, feedback) => sum + (feedback.pointMT || 0), 0);
        const teacherSTTotal = teacherSTFeedbacks.reduce((sum, feedback) => sum + (feedback.pointST || 0), 0);
